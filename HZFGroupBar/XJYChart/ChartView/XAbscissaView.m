...
- (void)dealData {
    [self.dataItemArray
        enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx,
            ...
            } else if ([obj isKindOfClass:[SFIMGroupBarItem class]]) {
                [self.dataDescribeArray addObject:((SFIMGroupBarItem *) obj).dataDescribe];
            }
        }];
}

- (void)setupUI {
    CGFloat labelWidth = self.frame.size.width / 6;
    //  CGFloat intervalWidth = labelWidth / 6;
    for (int i = 0; i < self.dataDescribeArray.count; i++) {
        XAlignLabel *label = [[XAlignLabel alloc]
            initWithFrame:CGRectMake(labelWidth * i, 0,
                                     labelWidth - 2,
